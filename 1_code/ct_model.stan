...
transformed parameters {
  ...
  // calculate group mean peak Ct and proliferation and clearance durations  
...
model {
  ...
  beta_dp ~ normal(0, priorsd);
  beta_wp ~ normal(0, priorsd);
  beta_wr ~ normal(0, priorsd);
  ...
  for (n in 1:N) {
    ...
    if (ct[n] >= lod) { // if above limit of detection
      ...
      log_lik[1] = log1m(sens) + exponential_lpdf(ctdrop[n] | 1 / fpmean);
      log_lik[2] = log(sens) + normal_lcdf(0 | mu[n], sigma);
    } else { // if below limit
      log_lik[1] = log1m(sens) + exponential_lpdf(ctdrop[n] | 1 / fpmean);
      log_lik[2] = log(sens) + normal_lpdf(ctdrop[n] | mu[n], sigma);
    }

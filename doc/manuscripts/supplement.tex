...
\newcommand{\suppfigpath}{../../output/figures}
...
\begin{appendix}
    ...
    Tables~\ref{tab:lfd} and~\ref{tab:symptom} report the posterior estimates of the LFD and symptom onset observation model parameters, respectively. Both models include post-peak indicator interaction terms that allow the relationship between viral load and the outcome to differ between the proliferation and clearance phases: the LFD model uses a logistic link (Section~3.3 of the main text) and the symptom model uses a discrete-time complementary log-log hazard (Section~3.5).
    ...
    \begin{figure}[p]
        \centering
        \includegraphics[width=\linewidth]{\suppfigpath/pnas/figS_stratified_curves.pdf}
    ...
    \begin{figure}[p]
        \centering
        \includegraphics[width=\linewidth]{\suppfigpath/pnas/fig8_conditional_symptom.pdf}
    ...
    \begin{figure}[p]
        \centering
        \includegraphics[width=\linewidth]{\suppfigpath/pnas/figS_site_comparison.pdf}
...
\subsection{Adopted approach: post-peak indicator interaction}

The LFD observation model uses a post-peak indicator interaction to allow the relationship between viral load and LFD sensitivity to differ between the rising and falling phases of infection:
\begin{equation*}
    \text{logit}\,P(L_t^* = 1) = \gamma_0 + \gamma_1 \log R_t + \gamma_2 \log V_t + \gamma_3 \mathbbm{1}(t \geq t_p) + \gamma_4 \mathbbm{1}(t \geq t_p) \log R_t
\end{equation*}
where $t_p$ is the individual's estimated RNA peak time. The effective model for LFD sensitivity is:
\begin{equation*}
    \text{logit}\,P(L_t^* = 1) = \begin{cases}
    ...
    \end{cases}
\end{equation*}
This is a natural extension of the regression framework already used throughout the model. It requires no additional functions or numerical computation: the peak time $t_p$ is already estimated for each individual as part of the RNA trajectory. The discontinuity at the peak is negligible in practice because LFD sensitivity is near its maximum at peak viral load, and the binary outcome (positive/negative) makes the logistic probability nearly indistinguishable on either side of the peak.
...
\paragraph{Derivative-based approach.} An alternative to the indicator interaction is to replace $\gamma_3 \mathbbm{1}(t \geq t_p) + \gamma_4 \mathbbm{1}(t \geq t_p) \log R_t$ with a single term $\gamma_3 \dot{g}_s(t)$, where $\dot{g}_s(t)$ is the time derivative of the smooth log-RNA trajectory. The derivative transitions smoothly from positive values during proliferation through zero at the peak to negative values during clearance, providing a continuous phase indicator with a single parameter. However, as a first-order correction that is additive in the logit, the derivative term cannot modulate the \emph{slope} of LFD sensitivity with respect to viral load---it only shifts the intercept. The indicator interaction, while discontinuous, is more flexible and fits naturally within the regression-based framework used throughout the model.
